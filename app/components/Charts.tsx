...
const Charts = async ({ params }) => {
  const res2 = await fetch(
    `https://api.open-meteo.com/v1/forecast?latitude=${params.latitude}&longitude=${params.longitude}&hourly=temperature_2m,precipitation_probability,uv_index,relativehumidity_2m&daily=uv_index_max&timezone=auto`,
    { next: { revalidate: 20 } }
  );
  const mainData = await res2.json();

  const hourly = mainData?.hourly?.time
    .map((e: string) =>
      new Date(e).toLocaleString("en-US", { hour: "numeric", hour12: true })
    )
    .slice(0, 24);
